...
app.controller('uigridController', ['$scope', '$http', function ($scope, $http) {
    $scope.gridOptions = {};
    ...
    //$http.get('/App/500_complex.json')
    //$http.get('http://localhost:63471/App/500_complex.json')
    ...
    for (var i = 0; i < datajson.length; i++) {
        datajson[i].registered = new Date(datajson[i].registered);
        datajson[i].gender = datajson[i].gender === 'male' ? 1 : 2;
              if (i % 2) {
            datajson[i].pet = 'fish'
            datajson[i].foo = { bar: [{ baz: 2, options: [{ value: 'fish' }, { value: 'hamster' }] }] }
              }
              else {
            datajson[i].pet = 'dog'
            datajson[i].foo = { bar: [{ baz: 2, options: [{ value: 'dog' }, { value: 'cat' }] }] }
              }
          }
    $scope.gridOptions.data = datajson;
